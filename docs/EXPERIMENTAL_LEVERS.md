...
| **Bond-formation accumulator (DDM)** | Replaces the instantaneous bond-formation *quality* read (`coherence_mean` / `allow_rate` disjunction) with a leaky asymmetric drift-diffusion accumulator (`agents/bond_accumulator.py`): a bond forms when *integrated* evidence crosses an accept bound — trickle/burst-immune by physics, with the deny-vs-earn asymmetry (60×, matching the trust economy's 40–80×) as geometry, and a three-state taxonomy (accept / active-reject / timeout) that separates structured hostility from noise. Structural preconditions (interactions ≥ 20, crystals ≥ 1) unchanged; commitment-only output — the field never sees the decision variable. Physics constants are architect-set placeholders pending Samuel's sign-off | OFF | validated (17-check invariant gate + 13-signature synthetic battery + live OFF/ON arm incl. the §6.3 reachable-range tripwire); opt-in until the architect sets the final constants | `CONFIG["bond_ddm_formation"] = True` (constants: `CONFIG["bond_ddm_config"]`, e.g. `{"trust_asymmetry": 80.0}`); tests: `tests.integration.bond_ddm_invariants` (CI), `tests.diagnostic.calibration.bond_ddm_synthetic_probe`, `bond_ddm_live_probe [steps] [dim]` | finding `2026-07-16-bond-formation-accumulator.md` |
